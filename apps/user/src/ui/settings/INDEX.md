...
## 职责
登录后的设置中心（WEB-FACE 面三），与管理后台严格分离。**浮在工作区之上的模态卡片面板**（通用 `@/ui/common/Modal`），非整页路由——打开不导航离开、语境原地保留，关闭走背幕/Esc/右上 ×。左 tab 竖导航统一 8 分区（账户 / 外观与语言 / 对话偏好 / 套餐目录 / 技能 / 连接 / 作品 / 团队），右内容区一次显一个 tab。开关态由 shell 持有并同步 URL `?settings=X`（刷新/深链/可分享）；`/settings?tab=X` 旧深链在路由层薄重定向到 `/?settings=X`。皮肤守 --k-* token，亮暗双态。
...
- `settings-sections.tsx`（账户/外观/对话/套餐目录卡）+ `settings-page.module.css`（sections 卡片/行/segment/select 皮肤）；账户/外观/对话为 sections，余额与目录分别复用只读 `BillingContent` / `PricingContent`。模态内两栏布局在 `settings-modal.module.css`。
