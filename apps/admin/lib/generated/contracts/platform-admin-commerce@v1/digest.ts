...
function commerceEnvelope(
  ...
  if (context.command === undefined) throw new Error("command_envelope_command_identity_missing");
  return authenticatedEnvelope("platform-admin-commerce@v1", operation, context, effect, targetRefs, verified);
}
function verifyCommerceEnvelope(
  ...
  const recomputed = commerceEnvelope(operation, context, effect, targetRefs, verified);
