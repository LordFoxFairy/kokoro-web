...
import {
  ...
  type OperatorScope,
} from "../../proto/kokoro/platform/admin/v2/admin_shared_pb.js";
...
} from "../../proto/kokoro/platform/credit/v1/admin_credit_pb.js";

function creditAdminEnvelope(operation: string, context: AuthenticatedOperatorCommandContext, effect: TypedPayload, targetRefs: readonly string[], verified: VerifiedAuthenticatedAdminAxes): string {
  if (context.command === undefined) throw new Error("command_envelope_command_identity_missing");
  return authenticatedEnvelope("platform-admin-credit@v1", operation, context, effect, targetRefs, verified);
}
function verifyCreditAdminEnvelope(operation: string, context: AuthenticatedOperatorCommandContext, effect: TypedPayload, targetRefs: readonly string[], verified: VerifiedAuthenticatedAdminAxes): string {
  const claimed = context.command?.requestDigest;
  if (claimed === undefined) throw new Error("command_envelope_command_identity_missing");
  const recomputed = creditAdminEnvelope(operation, context, effect, targetRefs, verified);
  if (recomputed !== claimed) throw new Error("command_envelope_request_digest_mismatch");
  return recomputed;
}
export function requestCreditReconciliationResolutionRequestDigest(context: CreditSiteCommandContext, effect: RequestCreditReconciliationResolutionEffect, verified: VerifiedAuthenticatedAdminAxes): string {
  if (context.operator === undefined) throw new Error("credit_site_authority_missing");
  return creditAdminEnvelope("kokoro.platform.credit.v1.AdminCreditService/RequestCreditReconciliationResolution", context.operator, {typeName: RequestCreditReconciliationResolutionEffectSchema.typeName, bytes: toBinary(RequestCreditReconciliationResolutionEffectSchema, effect, {writeUnknownFields: false})}, [context.siteId, effect.targetDigest], verified);
}
...
export function creditReconciliationDecisionRequestDigest(context: CreditSiteCommandContext, effect: CreditReconciliationDecisionEffect, verified: VerifiedAuthenticatedAdminAxes): string {
  if (context.operator === undefined) throw new Error("credit_site_authority_missing");
  return creditAdminEnvelope("kokoro.platform.credit.v1.AdminCreditService/DecideCreditReconciliationResolution", context.operator, {typeName: CreditReconciliationDecisionEffectSchema.typeName, bytes: toBinary(CreditReconciliationDecisionEffectSchema, effect, {writeUnknownFields: false})}, [context.siteId, effect.approvalRef], verified);
}
