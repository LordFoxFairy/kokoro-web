...
import {
  ProvisionedSiteState, RegisterSiteEffectSchema, SiteProvisioningService,
} from "@/lib/generated/proto/kokoro/platform/site/v1/site_provisioning_pb";
import {
  PublishedSiteReleaseState, PublishSiteReleaseEffectSchema, SitePublicationService,
} from "@/lib/generated/proto/kokoro/platform/site/v1/site_publication_pb";
import { CandidateAuthorityBindingSchema } from
  "@/lib/generated/proto/kokoro/platform/publication/v1/publication_common_pb";
import { registerSiteRequestDigest } from
  "@/lib/generated/contracts/platform-site-provisioning@v1/digest";
import { publishSiteReleaseRequestDigest } from
  "@/lib/generated/contracts/platform-site-publication@v1/digest";
import {
...
} from "@/lib/generated/proto/kokoro/platform/model/v1/model_control_pb";
import {
...
} from "@/lib/generated/contracts/platform-model-control@v1/digest";
...
export interface PublishSiteReleaseInput {
  readonly siteId: string; readonly candidateRef: string; readonly candidateVersion: string;
  readonly candidateAuthorizationEpoch: string; readonly candidateDigest: string; readonly reason: string;
}
...
export async function publishSiteRelease(input: PublishSiteReleaseInput) {
  const session = await requireAuthoritySession();
  const rpc = createClient(SitePublicationService, await adminControlPlaneTransport());
  const effect = create(PublishSiteReleaseEffectSchema, {
    candidate: create(CandidateAuthorityBindingSchema, {
      candidateRef: input.candidateRef,
      candidateVersion: strictPositiveUint64(input.candidateVersion),
      candidateAuthorizationEpoch: strictPositiveUint64(input.candidateAuthorizationEpoch),
  ...
  return committedMutation(context, () => rpc.publishSiteRelease({ context, siteId: input.siteId, effect },
    { headers: authHeaders(session) }), (response) => {
    const release = response.siteRelease;
    if (response.state !== PublishedSiteReleaseState.READY || response.siteId !== input.siteId ||
        release === undefined) throw invalidResponse();
    return { siteId: response.siteId, releaseRef: release.ref, releaseRevision: release.revision.toString(),
      releaseDigest: release.digest, state: "ready" as const, replayed: response.replayed,
      receipt: receiptJson(response.receipt) };
  });
}
...
async function queryCall(selection: ScopeSelection = { kind: "current" }) {
  ...
  return { query: createClient(AdminQueryService, transport), model: createClient(ModelControlService, transport),
...
function strictPositiveUint64(value: string): bigint {
  if (!/^[1-9][0-9]{0,19}$/u.test(value)) {
    throw new AdminControlPlaneError(Code.InvalidArgument, "site.release.candidate_uint64_invalid");
  }
