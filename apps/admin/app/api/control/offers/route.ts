...
const schema = z.object({ siteId, productRef: z.string().min(1).max(256), productVersionRef: z.string().min(1).max(256), productKind: z.enum(["credit_pack", "subscription", "bundle"]), revision: z.number().int().positive(), safeLabel: z.string().min(1).max(160), fulfillmentProgramRef: z.string().min(1).max(256), fulfillmentProgramRevisionRef: z.string().min(1).max(256), fulfillmentProgramRevision: z.number().int().positive(), plan: plan.optional(), outputs: z.array(output).min(1).max(100), legalTermRefs: z.array(z.string().min(1).max(256)).max(16) }).strict();
export async function GET(request: NextRequest) { try { return controlJson(await listOffers(siteId.parse(request.nextUrl.searchParams.get("siteId")), request.nextUrl.searchParams.get("pageToken") ?? undefined)); } catch (error) { return controlError(error); } }
export async function POST(request: NextRequest) { try { return controlJson(await publishOffer(schema.parse(await boundedJson(request))), { status: 201 }); } catch (error) { return controlError(error); } }
