...
Platform responses are exhaustively projected into the owner unions exported by `@kokoro/chat-surface`, so Chat, Studio, and Library render one owner-state read model without a parallel Generation/Job abstraction. Each generated Site injects its own brand name, browser runtime scope, CSRF, and theme while retaining an independent Web project, artifact, deployment, and domain. Studio and Library atomically hide/reset prior-scope state; every bootstrap, action, selection, recovery, and polling request is abortable and generation-fenced so an abort-insensitive stale completion cannot mutate the next account/project scope. Library polls only the selected Artifact's exact version owner list while visible; manual refresh atomically refreshes the artifact rail and that selected version list.
