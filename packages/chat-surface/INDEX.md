...
The exported media operation, candidate, artifact-version, failure, and credit-cost owner unions are the one browser read model shared with Studio and Library. Chat adds only Session-owned envelope fields where required; Platform projections never fabricate missing timestamps, operation links, storage URLs, or delivery authority.
