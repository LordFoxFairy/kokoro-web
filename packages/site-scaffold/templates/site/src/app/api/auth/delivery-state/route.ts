import { authRouteAllowed, prepareAuthDelivery, type AuthDeliveryPreparation } from "../../../../auth";
...
function preparation(value: unknown): Readonly<{ input: AuthDeliveryPreparation; advance: boolean }> | null {
  ...
  if (item.flow === "login" && typeof item.email === "string" && item.email.length <= 320 && typeof item.password === "string" && item.password.length <= 1024) {
    return { input: { flow: "login", email: item.email, password: item.password }, advance: item.advance === true };
  }
  if (item.flow === "mfa" && typeof item.transactionRef === "string" && item.transactionRef.length <= 256 && typeof item.code === "string" && item.code.length <= 64) {
