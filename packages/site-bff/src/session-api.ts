...
async function boundedJson(request: Request): Promise<unknown> {
  ...
  if (mediaType !== "application/json") throw new SyntaxError("unsupported media type")
...
function failure(error: unknown, context: Readonly<{
  ...
  if (error instanceof SiteBindingError && (
    error.code === "AUTH_SESSION_INVALID" || error.code.startsWith("PERSONAL_CONTEXT_")
  )) {
    return problem(401, "SESSION_ACCESS_GRANT_REQUIRED", "reauthenticate", "never", "Sign in again")
  }
...
export function createSiteSessionApi(input: Readonly<{
