...
function text(value: unknown, maximum = 2_048): string {
  if (typeof value !== "string" || value.length < 1 || value.length > maximum) throw new SyntaxError("text")
  return value
}
...
function command(value: unknown): Readonly<{ commandId: string; idempotencyKey: string }> {
  ...
  if (!zCommandIdentity.safeParse(commandId).success || !IDEMPOTENCY_KEY.test(idempotencyKey)) throw new SyntaxError("command")
  return Object.freeze({ commandId, idempotencyKey })
}

function mediaInput(value: unknown): MediaOperationInput {
  const parsed = zMediaOperationInput.safeParse(value)
  if (!parsed.success) throw new SyntaxError("media input")
  try {
...
function commandInput(value: unknown): Readonly<{
...
function cancellationInput(value: unknown): Readonly<{
  command: ReturnType<typeof command>
  ...
  if (!parsedCancellation.success) throw new SyntaxError("cancellation")
...
function pageQuery(url: URL): SiteMediaPageQuery {
  if ([...url.searchParams.keys()].some((key) => key !== "cursor" && key !== "limit")) throw new SyntaxError("query")
  if (url.searchParams.getAll("cursor").length > 1 || url.searchParams.getAll("limit").length > 1) throw new SyntaxError("query")
...
function contentUrl(artifactRef: string, artifactVersionRef: string): string {
...
function artifactProjection(artifact: ArtifactSummary) {
  return Object.freeze({
    ...artifact,
...
function versionProjection(version: ArtifactVersion) {
  return Object.freeze({
    ...version,
...
function delivery(url: URL): ArtifactDeliveryAuthorizationInput {
  const purposeValues = url.searchParams.getAll("purpose")
  if (purposeValues.length !== 1) throw new SyntaxError("purpose")
  const purpose = purposeValues[0]
  if (purpose === "preview") {
    if ([...url.searchParams.keys()].some((key) => key !== "purpose" && key !== "viewport")) throw new SyntaxError("query")
    const viewportValues = url.searchParams.getAll("viewport")
    if (viewportValues.length !== 1) throw new SyntaxError("viewport")
    const viewportClass = viewportValues[0]
    if (viewportClass !== "thumbnail" && viewportClass !== "canvas" && viewportClass !== "full") throw new SyntaxError("viewport")
    return Object.freeze({ purpose, viewportClass })
  }
  if (purpose === "download") {
    if ([...url.searchParams.keys()].some((key) => key !== "purpose" && key !== "filename")) throw new SyntaxError("query")
    const filenames = url.searchParams.getAll("filename")
    if (filenames.length > 1) throw new SyntaxError("filename")
  ...
  if (purpose === "export") {
    if ([...url.searchParams.keys()].some((key) => key !== "purpose" && key !== "exportIntentRef")) throw new SyntaxError("query")
    const exportIntentRefs = url.searchParams.getAll("exportIntentRef")
    if (exportIntentRefs.length !== 1) throw new SyntaxError("exportIntentRef")
    return Object.freeze({ purpose, exportIntentRef: text(exportIntentRefs[0], 256) })
  }
  throw new SyntaxError("purpose")
}

function range(value: string | null): ArtifactDeliveryByteRange | undefined {
  if (value === null) return undefined
  if (value.length > 64) throw new ArtifactDeliveryError("ARTIFACT_DELIVERY_RANGE_INVALID")
  const absolute = /^bytes=([0-9]+)-([0-9]+)$/u.exec(value)
  if (absolute !== null) return Object.freeze({ start: BigInt(absolute[1] ?? "0"), endInclusive: BigInt(absolute[2] ?? "0") })
  const suffix = /^bytes=-([1-9][0-9]*)$/u.exec(value)
  if (suffix !== null) return Object.freeze({ suffixLength: BigInt(suffix[1] ?? "0") })
  throw new ArtifactDeliveryError("ARTIFACT_DELIVERY_RANGE_INVALID")
}

type ReferenceSchema = Readonly<{ safeParse(value: unknown): Readonly<{ success: boolean }> }>

function reference(schema: ReferenceSchema, value: string | undefined): value is string {
  ...
  const api: SiteMediaApi = {
    async handle(request, path) {
      try {
        ...
        if (error instanceof ArtifactDeliveryError) return problem(416, error.code, "Artifact byte range was invalid")
        ...
        if (error instanceof PlatformPublicProtocolError) return problem(502, error.code, "Media upstream response was invalid")
        if (error instanceof ArtifactDeliveryProtocolError) return problem(502, error.code, "Artifact stream was rejected")
