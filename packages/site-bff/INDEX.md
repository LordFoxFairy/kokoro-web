...
Studio and Library use the exact allowlisted media/artifact composition, never a generic Platform proxy. The BFF resolves the Site's current project on the server for definitions, published model options, quote, submit, cancel, recover, operation reads, and artifact/version reads. Control input limits are enforced in UTF-8 bytes. Artifact content exists only for an exact ready owner version: the BFF verifies readiness, issues and immediately redeems a one-time capability on the server, validates owner byte size and Range metadata, and streams only the strict safe header allowlist, including a validated `bytes */size` 416. Delivery capability and Platform URL never enter browser JSON, React props, redirects, or content URLs.
