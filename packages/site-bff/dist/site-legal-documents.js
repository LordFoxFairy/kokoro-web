...
export function parseSiteLegalDocuments(raw) {
    ...
    try {
        ...
        const documents = value.map((candidate) => {
            ...
            if (Object.keys(entry).some((key) => !["termRef", "label", "href"].includes(key)))
                return null;
        ...
        if (!documents.every((document) => document !== null))
            return Object.freeze([]);
        const safe = documents;
        if (new Set(safe.map(({ termRef }) => termRef)).size !== safe.length)
            return Object.freeze([]);
        if (new Set(safe.map(({ href }) => href)).size !== safe.length)
            return Object.freeze([]);
...
export function publicLegalDocuments(documents) {
    return Object.freeze(documents.map(({ label, href }) => Object.freeze({ label, href })));
}
//# sourceMappingURL=site-legal-documents.js.map