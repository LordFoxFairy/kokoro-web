import "server-only";
import { zCommandIdentity, zMemoryCategory, zMemoryCorrectInput, zMemoryEntryRef, zMemoryExportInput, zMemoryExportRef, zMemoryForgetInput, zMemoryImportInput, zMemoryImportRef, zMemoryPriorityInput, zMemoryRememberInput, zMemoryResetInput, zMemoryRestoreInput, zMemoryRevisionRef, zMemorySettingsUpdateInput, zMemorySourceKind, } from "@kokoro/site-client";
import { PlatformPublicError, PlatformPublicInputError, PlatformPublicProtocolError, } from "@kokoro/site-client/server";
    ...
    return Object.freeze({
        ...
        remainingDeadlineMs() {
            ...
            if (!Number.isFinite(remaining) || remaining < 1)
                throw new Error("Memory request deadline exhausted");
...
function boundedResponse(value, status = 200) {
...
function outcomeUnknown(commandId) {
...
function parse(schema, value) {
    ...
    if (!result.success)
        throw new SyntaxError("contract input");
    return result.data;
}
...
async function mutationInput(request, schema) {
...
function uniqueQuery(url, allowed) {
    const result = {};
...
function pageQuery(url, memoryFilters) {
...
function knownRoute(method, path) {
    ...
    if (method !== "POST")
        return false;
    return ((path.length === 1 && (path[0] === "entries" || path[0] === "reset" || path[0] === "exports" || path[0] === "imports")) ||
...
export function createSiteMemoryAuthority(input) {
    const authority = {
...
export function createSiteMemoryApi(input) {
    return Object.freeze({
        async handle(request, path) {
            ...
            try {
                ...
                if (dispatchedCommandId !== undefined)
                    return outcomeUnknown(dispatchedCommandId);
