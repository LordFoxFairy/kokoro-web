...
export function createSiteAssetApi(input) {
    return Object.freeze({
        async handle(request, path) {
            try {
                ...
                if (request.method === "GET" && path.length === 1 && REFERENCE.test(path[0] ?? "")) {
                    const response = await input.runtime.getAssetUploadStatus(auth, path[0]);
                    return Response.json({ upload: uploadProjection(response.upload) }, {
                        headers: { "cache-control": "no-store" },
                    });
                }
                if (request.method === "POST" && path.length === 2 && path[1] === "complete" &&
                    REFERENCE.test(path[0] ?? "")) {
                    const parsed = completeInput(await boundedJson(request));
                    const response = await input.runtime.completeAssetUpload(auth, path[0], parsed.completion, parsed.command);
                    if (response.upload === null)
                        return problem(503, "OUTCOME_UNKNOWN", "Upload completion is reconciling");
                    return Response.json({ upload: uploadProjection(response.upload) }, {
                        status: 202,
                        headers: { "cache-control": "no-store" },
                    });
